...
class hparams:
    ...
    logging_dir = '../../models/TTS/Tacotron2/log/NVIDIA'
    last_ckpt = f"{model_output_path}/ckpt_{max(int(ckpt.split('_')[1]) for ckpt in os.listdir(model_output_path))}" \
        if os.path.exists(model_output_path) and os.listdir(model_output_path) else ""
